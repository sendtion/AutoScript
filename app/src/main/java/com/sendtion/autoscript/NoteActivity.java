...
public class NoteActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.bt_main_ShouShi).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
                    Toast.makeText(NoteActivity.this, "7.0及以上才能使用手势", Toast.LENGTH_SHORT).show();
                    return;
                }
                Path path = new Path();
                ...
                ClickButtonService.mService.dispatchGesture(new GestureDescription.Builder().addStroke(sd).build(), new AccessibilityService.GestureResultCallback() {
                    @Override
                    public void onCompleted(GestureDescription gestureDescription) {
                        super.onCompleted(gestureDescription);
                        Toast.makeText(NoteActivity.this, "手势成功", Toast.LENGTH_SHORT).show();
                    }

                    @Override
                    public void onCancelled(GestureDescription gestureDescription) {
                        super.onCancelled(gestureDescription);
                        Toast.makeText(NoteActivity.this, "手势失败，请重启手机再试", Toast.LENGTH_SHORT).show();
                    }
                }, null);
            }
        });
        ...
        findViewById(R.id.bt_main_DianJi).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AccessibilityNodeInfo ces = ClickButtonService.mService.findFirst(AbstractTF.newText("点击此区域加载下一页", true));
                if (ces == null) {
                    Utils.toast("找测试控件失败");
                    return;
                }
                ClickButtonService.clickView(ces);
            }
        });
        ...
        findViewById(R.id.bt_main_ChangAn).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                AccessibilityNodeInfo ces = ClickButtonService.mService.findFirst(AbstractTF.newText("测试控件", true));
                if (ces == null) {
                    Utils.toast("找测试控件失败");
                    return;
                }
...
//                ClickButtonService.mService.dispatchGestureClick(absXY.left + (absXY.right - absXY.left) / 2, absXY.top + (absXY.bottom - absXY.top) / 2);//手势点击效果
                //手势长按效果
                Path path = new Path();
                path.moveTo(absXY.left + (absXY.right - absXY.left) / 2, absXY.top + (absXY.bottom - absXY.top) / 2);//控件正中间
                ClickButtonService.mService.dispatchGesture(new GestureDescription.Builder().addStroke(new GestureDescription.StrokeDescription
        ...
        findViewById(R.id.bt_main_FanHui).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ClickButtonService.mService.performGlobalAction(AccessibilityService.GLOBAL_ACTION_BACK);
            }
        });
